...
void vector_swap(vector<int> &v1,vector<int> &v2, int start1, int end1, int start2, int end2) { 
    ...
    for(it1 = v1.begin()+start1; it1<v1.begin()+end1; it1++){
        v1_temp.push_back(*it1);
    }

    for(it2 = v2.begin()+start2; it2<v2.begin()+end2; it2++){
        v2_temp.push_back(*it2);
    }
    
    v1.erase(v1.begin()+start1,v1.begin()+end1);
    v1.insert(v1.begin()+start1,v2_temp.begin(),v2_temp.end());
    v2.erase(v2.begin()+start2,v2.begin()+end2);
    v2.insert(v2.begin()+start2,v1_temp.begin(),v1_temp.end());
